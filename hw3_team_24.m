...
function init()
    ...
    global block_size;
    global map_dim;
    global x_origin_block;
    global y_origin_block;
...
function isDone= update_current_map (status)
    ...
    isDone = false;
    block_xy = coord_to_block ([total_x_dist, total_y_dist]);
    
    x_idx = block_xy(1);
    y_idx = block_xy(2);
        
    before_value = global_map(x_idx, y_idx);
    
    if(before_value == status_boundary)
        return;
    end
    ...
    if(status == status_wall)
        global_map (x_idx, y_idx) = status_boundary;
        global_map_wall (x_idx, y_idx) = status_wall;
    elseif (stsatus == status_boundary);
        global_map (x_idx, y_idx) = status_boundary;
    else
        global_map (x_idx, y_idx) = status_explored;
    end
...
function draw_map ()
    ...
    g_mat = global_map == status_boundary;
...
function fill_blocks()
    ...
    boundary_map = (global_map + global_map_wall) == 1;
    filled_map = imfill(boundary_map,'hole');
    
    global_map = global_map | filled_map;
